...
def get_gps_atom(gps_atom_info,f):
    ...
    if data[12] == '\x05':
        ...
        try:
            ...
            print("Skipping: value error. Error: %s." % str(e))
            time = "2010-11-18T08:19:06Z"
            latitude = 0.0
            longitude = 0.0
            speed =0.0
...
def get_gpx(gps_data,out_file):
    ...
    for l in gps_data:
        if l:
            latlst.append(l[0])
            lonlst.append(l[1])
    medlat=median(latlst)
    medlon=median(lonlst)
    print medlat,medlon
    ...
    for l in gps_data:
        if l:
            print l[0],l[1]
            if l and l[0]>0.0 and l[1]>0.0 and ("2010" not in l[2]) and abs(medlat-l[0])<0.25 and abs(medlon-l[1])<0.25:
