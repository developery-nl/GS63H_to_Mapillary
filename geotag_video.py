...
def sample_video(video_file, image_path, sample_interval, file_type="video"):
    ...
    aname = os.path.basename(video_file)
    #aname = aname[:-4]
...
if __name__ == "__main__":
    ...
    #start time from filename , assumes format 2019_1217_115226_009.MP4
    aname = os.path.basename(video_file)
    aname = aname[:-8]  #formats to 2019_1217_115226
    print aname
    
    date_time_obj = datetime.datetime.strptime(aname, '%Y_%m%d_%H%M%S')
    print ("start time from filename: {}".format(date_time_obj))
    ...
    # FIX , use time from filename - 1 seconds
    ...
    print video_duration, gps_duration.total_seconds() 
    if abs(video_duration-gps_duration.total_seconds())>5:
        print "video length does not macth gps trace length in seconds"
    else:
